...
Blockly.Blocks['numpy_searching'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("[검색]")
        .appendField(new Blockly.FieldDropdown([["정규분포 랜덤표본(random.normal)", "random.normal"],["최대값 인덱스(argmax)","numpy_argmax"], ["최소값 인덱스(argmin)","numpy_Sorting_argmin"], ["조건 요소(where)","numpy_Searching_where"], ["균등분포 표본 추출(random.RandomStat().uniform)","random.RandomStat().uniform"], [" Pareto II 또는 Lomax 분포에서 샘플 그리기(random.pareto)","random.pareto"]]), "numpy_Searching_opt");
